...
import {ensureDir, mergeRecord, updatePackageJson, writeText} from './utils.js';

export const executeSvgr: StepExecutor = async (config, step) => {
  const changedFiles: string[] = [];

  changedFiles.push(await ensureDir(config.repoRoot, 'app/src/shared/assets/svg'));
  changedFiles.push(await ensureDir(config.repoRoot, 'app/src/shared/icons'));
  changedFiles.push(
...
async function main() {
  ...
  const svgFiles = (await fs.readdir(SVG_DIR)).filter((file) => file.endsWith('.svg'));
  ...
  const exportLines = ['// (auto-generated) Do not edit manually.', '// Run \`pnpm build:icons\` to regenerate.\\n'];
  ...
  for (const file of generatedFiles) {
    ...
    content = content.replace(/(stroke)=['"]([^'"]+)['"]/g, '$1="currentColor"');
    content = content.replace(/(<path[^>]*?|<g[^>]*?)fill=['"]([^'"]+?)['"]/g, (match, start, fillValue) => {
      if (fillValue === 'none' || fillValue === 'transparent' || fillValue.startsWith('url(')) {
        return match;
      }
    ...
    content = content.replace(/export default Svg[^;]+;/, \`export default \${componentName};\`);
    await fs.writeFile(newPath, content, 'utf8');
    exportLines.push(\`export {default as \${componentName}} from './\${newFileName.replace('.tsx', '')}';\`);
  }
...
main().catch((error) => {
  ...
  return {
    stepId: step.id,
    changedFiles,
    message: 'SVGR icon build pipeline was configured.',
  };
};
